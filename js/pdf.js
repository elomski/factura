...
/* ══════════════════════════════════════════════════════
   BON DE RAVITAILLEMENT — document dédié, format A4 fixe.
   Distinct de genererPDF() (ventes/devis/factures) : pas de
   TVA, pas de mode de paiement, un fournisseur au lieu d'un
   client, et un tableau simple avec la pagination native
   d'autoTable (pas de mise en page adaptative multi-format).
══════════════════════════════════════════════════════ */
function genererPDFRavitaillement(data, entreprise) {
  const { jsPDF } = window.jspdf;
  const S  = str => sanitizePdf(str);
  const mg = 14, pw = 210, ph = 297, lw = pw - 2 * mg;
  const dev = entreprise.devise || "F CFA";

  const doc = new jsPDF({ unit: "mm", format: "a4", orientation: "portrait" });
  let y = mg;

  /* ── En-tête : logo + entreprise ── */
  const hasLogo = !!entreprise.logoUrl;
  if (hasLogo) {
    try { doc.addImage(entreprise.logoUrl, "JPEG", mg, y, 22, 22, undefined, "FAST"); }
    catch (_) { /* logo illisible — on continue sans bloquer le PDF */ }
  }
  const txX = hasLogo ? mg + 26 : mg;
  doc.setFont("helvetica", "bold"); doc.setFontSize(15); doc.setTextColor(..._C.NOIR);
  doc.text(S(entreprise.nom || "Entreprise"), txX, y + 6);
  doc.setFont("helvetica", "normal"); doc.setFontSize(8); doc.setTextColor(..._C.MUTED);
  let iy = y + 11;
  [
    [entreprise.adresse, entreprise.ville, entreprise.pays].filter(Boolean).join(", "),
    [entreprise.tel, entreprise.tel2].filter(Boolean).join(" / "),
    entreprise.email,
  ].filter(Boolean).forEach(l => { doc.text(S(l), txX, iy); iy += 4; });

  y = Math.max(y + 24, iy + 2);

  doc.setDrawColor(..._C.GRIS_LINE); doc.setLineWidth(0.3);
  doc.line(mg, y, pw - mg, y);
  y += 8;

  /* ── Titre document ── */
  doc.setFont("helvetica", "bold"); doc.setFontSize(18); doc.setTextColor(..._C.CUIVRE);
  doc.text("BON DE RAVITAILLEMENT", mg, y);
  doc.setFont("helvetica", "normal"); doc.setFontSize(10); doc.setTextColor(..._C.NOIR2);
  doc.text("N° " + S(data.numero ?? ""), pw - mg, y, { align: "right" });
  y += 8;

  /* ── Bandeau date / fournisseur ── */
  const dObj = data.date instanceof Date ? data.date : toDateObj(data.date);
  const dStr = String(dObj.getDate()).padStart(2, "0") + "/" +
               String(dObj.getMonth() + 1).padStart(2, "0") + "/" + dObj.getFullYear();
  const rows3 = [
    { l: "Date :", v: dStr },
    data.fournisseur ? { l: "Fournisseur :", v: data.fournisseur } : null,
  ].filter(Boolean);
  doc.setFillColor(..._C.GRIS_BG);
  doc.rect(mg, y, lw, rows3.length * 6 + 3, "F");
  let ty = y + 3;
  rows3.forEach(r => {
    doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(..._C.MUTED);
    doc.text(r.l, mg + 2, ty + 3.5);
    doc.setFont("helvetica", "bold"); doc.setTextColor(..._C.NOIR);
    doc.text(S(r.v), pw - mg - 2, ty + 3.5, { align: "right" });
    ty += 6;
  });
  y += rows3.length * 6 + 8;
  ...
  const rows = lignes.map(l => ({
    des: S(l.produitNom || "—"),
    qte: String(l.qte),
    pu:  _fmtNum(l.prixUnitaire || 0),
    tot: _fmtNum(l.total ?? (l.qte * (l.prixUnitaire || 0))),
  }));

  doc.autoTable({
    startY: y,
    margin: { left: mg, right: mg, top: mg, bottom: mg + 24 },
    columns: [
      { header: "ARTICLE",     dataKey: "des" },
      { header: "QTÉ REÇUE",   dataKey: "qte" },
      { header: "P.U. ACHAT",  dataKey: "pu"  },
      { header: "MONTANT",     dataKey: "tot" },
    ],
    body: rows,
    columnStyles: {
      des: { cellWidth: lw * 0.46, halign: "left"   },
      qte: { cellWidth: lw * 0.16, halign: "center" },
      pu:  { cellWidth: lw * 0.18, halign: "right"  },
      tot: { cellWidth: lw * 0.20, halign: "right", fontStyle: "bold" },
    },
    styles: {
      font: "helvetica", fontSize: 9, cellPadding: 2.2,
      textColor: _C.NOIR2, lineColor: _C.GRIS_LINE, lineWidth: 0.15,
    },
    headStyles: { fillColor: _C.CUIVRE, textColor: _C.BLANC, fontStyle: "bold", fontSize: 8.5 },
    alternateRowStyles: { fillColor: _C.GRIS_ALT },
  });

  y = doc.lastAutoTable.finalY + 6;
  if (y > ph - mg - 40) { doc.addPage(); y = mg; }

  /* ── Total ── */
  const totalAchat = data.totalAchat ?? lignes.reduce((s, l) => s + (l.total || 0), 0);
  doc.setFillColor(..._C.TOT_BG);
  doc.rect(pw - mg - 70, y, 70, 10, "F");
  doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(..._C.TOT_LBL);
  doc.text("TOTAL", pw - mg - 66, y + 6.5);
  doc.setFontSize(11); doc.setTextColor(..._C.TOT_TXT);
  doc.text(_fmtNum(totalAchat) + " " + dev, pw - mg - 4, y + 6.5, { align: "right" });
  y += 16;

  /* ── Note ── */
  if (data.note) {
    doc.setFont("helvetica", "italic"); doc.setFontSize(8.5); doc.setTextColor(..._C.MUTED);
    doc.splitTextToSize(S("Note : " + data.note), lw).forEach(l => { doc.text(l, mg, y); y += 4; });
    y += 2;
  }
  ...
  const bW   = (lw - 8) / 2;
  [
    { x: mg,           label: "RÉCEPTIONNÉ PAR" },
    { x: mg + bW + 8,  label: "FOURNISSEUR"      },
  ].forEach(b => {
    doc.setDrawColor(..._C.GRIS_LINE); doc.setFillColor(..._C.GRIS_BG); doc.setLineWidth(0.2);
    doc.roundedRect(b.x, sigY, bW, 20, 1.5, 1.5, "FD");
    doc.setFont("helvetica", "bold"); doc.setFontSize(7.5); doc.setTextColor(..._C.MUTED);
    doc.text(b.label, b.x + bW / 2, sigY + 5, { align: "center" });
  });

  /* ── Pied de page ── */
  doc.setFont("helvetica", "normal"); doc.setFontSize(6); doc.setTextColor(210, 205, 200);
  doc.text("FacturaPro", pw / 2, ph - 6, { align: "center" });

  const fname = S(data.numero ?? "ravitaillement") + "_" +
    S((entreprise.nom || "facturapro").replace(/\s+/g, "_")) + ".pdf";
  doc.save(fname);
  return fname;
}