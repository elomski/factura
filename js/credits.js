...
window.enregistrerPaiement = async function enregistrerPaiement() {
  ...
  loader(true);
  try {
    const venteRef     = db.collection("ventes").doc(_creditEnCours.id);
    const paiementsRef = venteRef.collection("paiements");
    const nouveauPaiementRef = paiementsRef.doc(); // ID généré à l'avance

    let soldeAtteint = false;

    // [FIX P1-5] Création du paiement + mise à jour du solde dans UNE
    // SEULE transaction (soit les deux réussissent, soit aucun) — et
    // la vente est relue fraîchement pour bloquer un paiement sur une
    // ardoise déjà soldée ou une vente annulée entre-temps.
    await db.runTransaction(async transaction => {
      ...
      if (venteData.annulee === true) throw new Error("Cette vente a été annulée.");
      if (venteData.solde === true) throw new Error("Cette ardoise est déjà entièrement soldée.");

      transaction.set(nouveauPaiementRef, {
        montant,
        note,
        dateStr,
      ...
      if (nouveauSolde <= 0.01) {
        soldeAtteint = true;
        transaction.update(venteRef, { solde: true });
      }
    });
    ...
    if (soldeAtteint) {
      ...
      toast(`✅ Paiement de ${fmt(montant)} enregistré. Reste : ${fmt(_creditEnCours.soldeRestant - montant)}`);
    }
...
window.marquerSolde = async function marquerSolde(ventId) {
  ...
  try {
    const c = allCredits.find(x => x.id === ventId);
    const venteRef     = db.collection("ventes").doc(ventId);
    const paiementsRef = venteRef.collection("paiements");

    // [FIX P1-5] Même logique atomique que enregistrerPaiement().
    await db.runTransaction(async transaction => {
      const venteSnap = await transaction.get(venteRef);
      if (!venteSnap.exists) throw new Error("Vente introuvable.");
      const venteData = venteSnap.data();
      if (venteData.annulee === true) throw new Error("Cette vente a été annulée.");
      if (venteData.solde === true) return; // déjà soldée, rien à faire

      if (c && c.soldeRestant > 0.01) {
        const nouveauPaiementRef = paiementsRef.doc();
        transaction.set(nouveauPaiementRef, {
