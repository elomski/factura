...
window.VUES_PAR_ROLE = {
  // [FIX STOCK-CAISSIER] Le caissier a maintenant accès au catalogue
  // pour CONSULTER le stock disponible (lecture seule — voir
  // _applyRoleRestrictions dans shop.js et renderCatalogue dans
  // catalogue.js pour le masquage des actions de modification).
  caissier: ["dashboard", "nouvelle-vente", "historique", "catalogue"],
...
window.supprimerVente = async function supprimerVente(id) {
  // [FIX STOCK-CAISSIER] Annuler une vente restitue du stock (le fait
  // augmenter) — réservé à gérant/propriétaire, comme dans les règles
  // Firestore. Sans ce garde-fou, un caissier obtiendrait une erreur
  // "permission denied" peu claire au lieu d'un message explicite.
  if (window.userRole === 'caissier') {
    toast("⛔ Seul un gérant ou le propriétaire peut annuler une vente.", "err");
    return;
  }
